import os, uuid, boto3, json, requests
...
# Get Load Balancer URL from environment variable with fallback
LOAD_BALANCER_DNS = os.getenv('FRACTAL_SERVICE_URL', 'http://localhost:8080')
FRACTAL_SERVICE_URL = LOAD_BALANCER_DNS

print(f"🔗 Fractal Service URL: {FRACTAL_SERVICE_URL}")
    ...
    try:
        ...
        for message in messages:
            try:
                ...
                # Call fractal service through load balancer
                fractal_response = requests.post(
                    f"{FRACTAL_SERVICE_URL}/generate",
                    data=body,
                    timeout=300  # 5 minute timeout for fractal generation
                )
...
async def generate_fractal(
    ...
    # Fallback: Direct call to fractal service through load balancer
    try:
        fractal_response = requests.post(
            f"{FRACTAL_SERVICE_URL}/generate",
            data={
